            ...
            @foreach ($users as $user)
                ...
                <div class="mt-12 flex flex-col items-center text-center gap-8">
                    ...
                    <div class="flex flex-col w-full md:w-1/2 gap-4 items-start">
                        @foreach ($authuser->availedPricingPlan as $pricingPlan)
                            @if ($pricingPlan->availed_to_id == $service->user_id && $pricingPlan->has_review == false)
                                <x-home.user-form-comment :authuser="$authuser" :user="$user" :service="$service"/>
                            @else
                                <span class="w-full">You have to avail first before adding a review.</span>
                            @endif
                        @endforeach
